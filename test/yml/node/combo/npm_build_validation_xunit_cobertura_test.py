...
fileName = "npm-build-validation-xunit-cobertura.yml"
...
runNpmBuildScriptParameters = parameters["runNpmBuildScript"]
...
runNpmBuildScriptStep = steps[1]
runNpmBuildScriptStepParameters = runNpmBuildScriptStep["parameters"]
...
def test_run_npm_build_script_step_script_name_parameter_default():
    assert runNpmBuildScriptParameters["buildNpmScriptName"] == "build"


def test_run_npm_build_script_step_display_name_parameter_default():
    assert runNpmBuildScriptParameters["taskDisplayName"] == "Run build script"
...
def test_npm_install_step_display_name_parameter():
    value = npmInstallParameters["taskDisplayName"]
    assert value == "${{ parameters.npmInstall.taskDisplayName }}"


def test_npm_run_build_script_step_template_path():
    value = runNpmBuildScriptStep["template"]
    assert value == "../steps/simple/npm-run-build.yml"


def test_npm_run_build_script_build_script_name_parameter():
    value = runNpmBuildScriptStepParameters["buildNpmScriptName"]
    assert value == "${{ parameters.runNpmBuildScript.buildNpmScriptName }}"


def test_npm_install_step_display_name_parameter():
    value = runNpmBuildScriptStepParameters["taskDisplayName"]
    assert value == "${{ parameters.runNpmBuildScript.taskDisplayName }}"
