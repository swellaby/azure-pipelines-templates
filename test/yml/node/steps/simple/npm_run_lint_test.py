...
contents = parse_node_simple_step_template_yaml_file("npm-run-lint.yml")
...
def test_task_lint_npm_script_name_parameter_default():
    assert parameters["lintNpmScriptName"] == "lint"
...
def test_step_custom_command_input():
    ...
    assert value == "run ${{ parameters.lintNpmScriptName }}"
